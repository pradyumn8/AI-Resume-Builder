...
const ProfrssionalSummaryForm = ({ data, onChange, setResumeData }) => {
    ...
    const generateSummary = async () => {
        try {
            ...
            const prompt = `enchance my professinal summary "${data}"`;
    ...
    return (
        <div className='space-y-4'>
            <div className='flex items-center justify-between'>
                ...
                <button disabled={isGenerating} onClick={generateSummary} className='flex items-center gap-2 px-3 py-1 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50'>
            ...
            <div className='mt-6'>
                <textarea rows={7} value={data} onChange={(e)=>onChange(e.target.value)} className='w-full p-3 px-4 mt-2 border text-sm border-gray-300 rounded-lg focus:ring focus:ring-blue-500 outline-none transition-colors resize-none' placeholder='Write a compelling professinal summary that highlights your key strengths and career objectives...'></textarea>
