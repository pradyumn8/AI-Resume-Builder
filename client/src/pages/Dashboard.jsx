import { FilePenLineIcon, PencilIcon, PlusIcon, Trash, TrashIcon, UploadCloud, UploadCloudIcon, XIcon } from 'lucide-react'
    ...
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='flex gap-4'>
          <button onClick={() => setShowCreateResumes(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed hover:border-2px-solid hover:shadow-lg transition-all duration-300'>
            <PlusIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br from-indigo-300 to-indigo-500 text-white rounded-full' />
            <p className='text-sm group-text-purple-600 transition-all duration-300'>Create Resume</p>
          </button>
          <button onClick={() => setShowUploadResumes(true)} className='w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed hover:border-2px-solid hover:shadow-lg transition-all duration-300'>
            <UploadCloudIcon className='size-11 transition-all duration-300 p-2.5 bg-gradient-to-br from-purple-300 to-purple-500 text-white rounded-full' />
            <p className='text-sm group-text-purple-600 transition-all duration-300'>Upload Existing</p>
          </button>
        </div>
        ...
        <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          {allResumes.map((resume, index) => {
            ...
            return (
              <button key={index} onClick={()=> navigate(`builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{ background: `linear-gradient(135deg, ${baseColor}20 0%, ${baseColor}10 100%)`, borderColor: baseColor + '40' }}>
                ...
                <p className='absolute bottom-1 text-[11px] text-slate-400 group-hover:text-slate-600 transition-all duration-300 px-2 text-center' style={{ color: baseColor + `90` }}>
        ...
        {showUploadResumes && (
          <form onSubmit={uploadResume} onClick={() => setShowUploadResumes(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
              ...
              <button className='w-full py-2 bg-green-600 text-white rounded hover:bg-green-700 transition-colors' onClick={()=>{setShowUploadResumes(false); setTitle('')}}>Uplaod Resume</button>
